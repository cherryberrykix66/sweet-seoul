...
CREATE TABLE ORDERS (
    ...
    CONSTRAINT fk_orders_customer_id FOREIGN KEY (customer_id) 
        REFERENCES CUSTOMERS(customer_id),
    CONSTRAINT fk_orders_marketing_channel FOREIGN KEY (marketing_channel_id) 
        REFERENCES MARKETING_CHANNELS(marketing_channel_id),
    CONSTRAINT fk_orders_gift_card_id FOREIGN KEY (gift_card_id) 
        REFERENCES GIFT_CARDS(gift_card_id),
    -- Check Constraints with unique names
    CONSTRAINT chk_orders_subtotal_positive CHECK (subtotal >= 0),
    CONSTRAINT chk_orders_tax_positive CHECK (tax_amount >= 0),
    CONSTRAINT chk_orders_shipping_positive CHECK (shipping_cost >= 0),
    CONSTRAINT chk_orders_discount_positive CHECK (discount_amount >= 0),
    CONSTRAINT chk_orders_total_positive CHECK (total_amount >= 0),
    CONSTRAINT chk_orders_status_valid CHECK (order_status IN ('Pending', 'Processing', 'Shipped', 'Delivered', 'Canceled')),
    CONSTRAINT chk_orders_platform_valid CHECK (platform IN ('online', 'in-store')),
    CONSTRAINT chk_orders_gift_flag CHECK (is_gift IN (0, 1)),
    CONSTRAINT chk_orders_payment_valid CHECK (payment_method IN ('Credit Card', 'Debit Card', 'PayPal', 'Gift Card', 'Cash', 'Apple Pay')),
    CONSTRAINT chk_orders_total_calc CHECK (total_amount = subtotal + tax_amount + shipping_cost - discount_amount)
);
...
CREATE INDEX idx_orders_marketing_channel ON ORDERS(marketing_channel_id);

-- Verify the table was created successfully
SELECT 'ORDERS table created successfully' AS status FROM dual;

-- Check constraint names to verify uniqueness

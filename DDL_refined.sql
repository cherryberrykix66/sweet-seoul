...
CREATE TABLE INVENTORY (
    ...
    storage_requirements VARCHAR2(1000),
...
CREATE TABLE CUSTOMER_PREFERENCES (
    ...
    flavor_preference VARCHAR2(1000),
    product_category_preference VARCHAR2(1000),
...
CREATE TABLE FEEDBACK (
    ...
    CONSTRAINT chk_is_verified CHECK (is_verified_purchase IN (0, 1)),
    CONSTRAINT chk_feedback_association CHECK (order_id IS NOT NULL OR product_id IS NOT NULL)
);
...
CREATE TABLE ORDER_ITEMS (
    ...
    CONSTRAINT chk_discount_reasonable CHECK (discount <= unit_price * quantity * 1.1)
);
...
CREATE TABLE SUBSCRIPTION_ITEMS (
    ...
    created_at TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
    updated_at TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT fk_sub_item_subscription FOREIGN KEY (subscription_id) 
        REFERENCES SUBSCRIPTIONS(subscription_id) ON DELETE CASCADE,
    CONSTRAINT fk_sub_item_product FOREIGN KEY (product_id) 
        REFERENCES PRODUCTS(product_id),
    CONSTRAINT uq_sub_product_delivery UNIQUE (subscription_id, product_id, delivery_date),
    CONSTRAINT chk_quantity_positive CHECK (quantity > 0),
    CONSTRAINT chk_delivery_status CHECK (delivery_status IN ('Planned', 'Shipped', 'Delivered', 'Skipped')),
    CONSTRAINT chk_delivery_date_range CHECK (
...
CREATE TABLE PRODUCT_PROMOTIONS (
    ...
    created_at TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
    updated_at TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
    CONSTRAINT fk_prod_promo_product FOREIGN KEY (product_id) 
        REFERENCES PRODUCTS(product_id) ON DELETE CASCADE,
    CONSTRAINT fk_prod_promo_promotion FOREIGN KEY (promotion_id) 
        REFERENCES PROMOTIONS(promotion_id) ON DELETE CASCADE,
    CONSTRAINT uq_product_promotion UNIQUE (product_id, promotion_id),
    CONSTRAINT chk_override_discount CHECK (override_discount_amount IS NULL OR override_discount_amount >= 0),
...
-- Trigger for ORDER_STATUS_HISTORY when ORDERS.order_status changes
...
BEGIN
    INSERT INTO ORDER_STATUS_HISTORY (order_id, previous_status, new_status, updated_by)
    VALUES (:NEW.order_id, NULL, :NEW.order_status, USER);
END;
/

-- Trigger to update gift card balance when used in an order
...
BEGIN
    IF :NEW.gift_card_id IS NOT NULL THEN
        ...
        IF v_current_balance < :NEW.total_amount THEN
            RAISE_APPLICATION_ERROR(-20003, 'Insufficient gift card balance');
        END IF;
...
-- Customer promotion usage trigger
...
-- ORDER_ITEMS table update trigger
...
-- SUBSCRIPTION_ITEMS table update trigger
...
-- ORDER_STATUS_HISTORY table update trigger
...
-- PRODUCT_PROMOTIONS table update trigger
...
PROMPT
PROMPT =====================================================
PROMPT K-pop/Anime Merchandise Database is ready for use!
PROMPT =====================================================